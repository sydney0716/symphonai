...
class OpenAIProvider(ModelProvider):
    ...
    def create_response(self, request: ModelRequest) -> ModelResponse:
        ...
        try:
            ...
            raise ProviderError(f"OpenAI API returned HTTP {exc.code}: {detail}") from None
        except urllib.error.URLError as exc:
            raise ProviderError(f"OpenAI API request failed: {exc.reason}") from None
        except TimeoutError:
            raise ProviderError(f"OpenAI API request timed out after {self.timeout_seconds}s") from None
